...
workspace): the packages/plugins bundle to `dist/` with `.d.ts`, the api
transpiles file-for-file (`unbundle`) so `dist/http/main.js` stays the process
entry. `tsc` never emits — it is the typechecker (`pnpm typecheck`).
...
re-export of a workspace package (see `plugins/slack`).
