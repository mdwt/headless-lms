...
- `core/<context>/` — framework-free, runtime-free, **persistence-free** domain. Seven bounded contexts, all built and Drizzle-persisted: `identity`, `organizations`, `courses`, `entitlements`, `progress`, `assets`, `integrations` (+ `shared/` for cross-cutting ports). Integration implementations (`stripe`, `slack`) live in `src/integrations/` (outside `adapters/` — one folder per integration, **directory name = integration id**), each default-exporting a module satisfying the core `Integration` port. Composition scans that directory at startup (`loadIntegrationsRegistry`) — adding an integration is adding a folder; nothing else to register.
