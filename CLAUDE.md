...
- **Specced resources** (7 tags / 26 operations): `courses`, `modules` (course sub-resource), `students`, `enrollments`, `submissions`, `team`, `dashboard` (overview). These mirror the `apps/admin` dashboard surface.
- The back-office contexts (`courses`, `modules`, `students`, `enrollments`, `submissions`, `team`, `dashboard`) are backed by **in-memory repositories** (`adapters/inmemory/*`) because their Drizzle tables are still stubs — seeded deterministically, mutations live for process life. The core/port/route/SDK layers are real; swapping in a Drizzle repo is a one-line change per context in `composition/container.ts`. They are deliberately separate contexts from the auth-wired `identity`/`organizations` so the back-office read models don't disturb auth.
