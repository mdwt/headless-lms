// team — Drizzle read repository. Reads the domain mirror of the org's members
// (memberships) and pending invitations, joined to the student/user for display.
// Writes go through the auth provider (see adapters/auth/org-admin.ts).
import { and, eq } from "drizzle-orm";
import type { NodePgDatabase } from "drizzle-orm/node-postgres";
import type { TeamRepository, TeamMemberRecord } from "../../../core/team/index.js";
import type { Member, MembersQuery, Page, Role } from "../../../core/team/index.js";
...
export class DrizzleTeamRepository implements TeamRepository {
  ...
  private async loadAll(orgId: string): Promise<TeamMemberRecord[]> {
    ...
    const invited: TeamMemberRecord[] = inviteRows.map((i) => ({
      id: i.id,
      name: i.email,
      email: i.email,
      image: null,
  ...
  async findByEmail(orgId: string, email: string): Promise<TeamMemberRecord | null> {
    const all = await this.loadAll(orgId);
    return all.find((r) => r.email.toLowerCase() === email.toLowerCase()) ?? null;
  }

  async findById(orgId: string, id: string): Promise<TeamMemberRecord | null> {
    const all = await this.loadAll(orgId);
