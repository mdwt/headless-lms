// enrollments — Drizzle repository (implements the core outbound port). Org-scoped.
...
const derivedStatus = sql<EnrollmentStatus>`case
...
export class DrizzleEnrollmentsRepository implements EnrollmentsRepository {
  ...
  async list(orgId: string, query: EnrollmentsQuery): Promise<Page<Enrollment>> {
  ...
  async insert(orgId: string, input: GrantEnrollmentInput): Promise<Enrollment> {
    ...
    if (!row) throw new Error("failed to insert enrollment");
    const enrollment = await this.findById(orgId, row.id);
    if (!enrollment) throw new Error("failed to read inserted enrollment");
    return enrollment;
  }
